  ...
  function createAgent(tenant, opts) {
    ...
    function handleMessage(sessionId, input) {
      ...
      if (buttonId) {
        ...
        // בוואטסאפ כפתורים ישנים חיים לנצח בהיסטוריית הצ'אט — לחיצה על כפתור
        // שכבר לא רלוונטי לשלב חייבת תשובה, לא null (שמפיל את השולח).
        if (!out) {
          out = reply('הכפתור הזה כבר לא בתוקף. 🙂 איך אפשר לעזור?', menuButtons());
        }
      } else {
